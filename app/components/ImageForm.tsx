...
interface ImageFormProps {
  ...
  onChange: (attributes: StarwarsCharacters) => void;
  imageUrl: string;
}

// Komponenta sprejema tri propse
const ImageForm: React.FC<ImageFormProps> = ({ initialAttributes, onChange, imageUrl }) => {
  const [attributes, setAttributes] = useState<StarwarsCharacters>(initialAttributes);
  const [isEditMode, setIsEditMode] = useState<boolean>(false);

  const handleChange = (attributeName: keyof StarwarsCharacters) => (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    <>
      ...
      >
        ...
        <div className="w-full flex items-end">
          <form onSubmit={(e) => e.preventDefault()} className="w-full relative bg-white bg-opacity-0 rounded-lg p-8">
